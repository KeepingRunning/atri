...
内存历史按时间清理，重启也只恢复窗口内的正文。完整 JSONL、消息去重记录和已发送消息 ID 仍保留；窗口之外的旧记录不会发给 LLM。排队等待期间跨过一小时边界的历史，会在实际构造上下文时排除。旧 `context.recent_messages` 已停用、会被忽略；已有配置已改为 `history_seconds`，修改配置或升级代码后重启服务生效。
