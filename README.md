...
Planner 模式先正常发送文字，确认成功后由独立表情规划器结合本轮快照、已发正文和本地候选，决定是否单独补一张图。补图不影响正文生成，默认 20 秒内未提交即丢弃，新消息会取消尚未提交的补图。每群平均三至五轮自然发言补图一次作为软目标，文字与图片分别记录回执、共同计为一轮。

本地配置已启用，当前 170 张素材中 167 张可用，3 张待复核暂不参与发送。图片和描述在 `data/sticker_review/`；模板默认关闭。执行 `uv run atri check` 检查素材，重启后试聊。工具接口、频率、动图与容器迁移说明见 [表情包发送](docs/stickers.md)。暂不增加无新消息时主动开场的任务。
