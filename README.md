...
Planner 模式先正常发送文字，确认成功后由同一个辅助表达规划器结合本轮快照、已发正文和本地候选，选择不补充、单独补一张图或一条日语语音。每轮最多补充一种，默认 20 秒内未提交即丢弃，新消息取消尚未提交的补充。合计每群平均三至五轮自然发言补充一次，其中语音六至十轮作为软目标；文字与媒体分别记录回执、共同计为一轮。

图片素材在 `data/sticker_review/`，语音素材与保留清单在 `data/voice_review/`；发行模板默认关闭。语音按日中原句、用途和语境前提选择，短句优先，相同台词的不同录音也会降权。待标注、待复核和纯标点语音留库但暂不自动发送。执行 `uv run atri check` 查看当前可用数量，更新配置或素材后重启生效。接口、频率与部署说明见 [辅助表达](docs/supplements.md)。

## 语音素材审阅

日文原声素材的文本对照、表达描述和试听筛选见 [语音素材审阅](docs/voice-preprocessing.md)。当前 2,209 条原声全部保留；是否参与群聊由 `[voices].enabled` 和素材标注状态控制，预处理命令本身不发送消息。

推荐通过本地 HTTP 在外部浏览器试听，避开 VS Code 文件预览的媒体加载问题：
